...
while should_continue:
    choice = input("What do you want to generate? Type '1' for graph or '2' for numbers\n").lower
    choice2 = int(
    ...
    if choice == 1 & choice2 == 1:
        ...
        plt.show()
    elif choice == 1 & choice2 == 5:
        df.loc[:, "Survived"][df.Survived == 0] = "no"
        df.loc[:, "Survived"][df.Survived == 1] = "yes"
        sbn.catplot(data=df, x='Pclass', kind='count', hue='Survived')
        plt.show()
    elif choice == 2 & choice2 == 1:
        print(df.Sex.value_counts())
    elif choice == 2 & choice2 == 2:
        print(df.Survived.value_counts())
    elif choice == 2 & choice2 == 3:
    ...
    elif choice == 2 & choice2 == 4:
        df["Age"] = df["Age"].fillna(-0.5)
        bins = [-1, 0, 5, 12, 18, 24, 35, 60, np.inf]
        labels = ['Unknown', 'Baby', 'Child', 'Teenager', 'Student', 'Young Adult', 'Adult', 'Senior']
        df['AgeGroup'] = pd.cut(df["Age"], bins, labels=labels)
        print(df['AgeGroup'].value_counts())
    elif choice == 2 & choice2 == 5:
